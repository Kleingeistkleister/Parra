...
class MCU_stepper:
    def __init__(self, name, step_pin_params, dir_pin_params,
        ...
        self._dmx_channels = 4
        self._dmx_values = [0] * self._dmx_channels
        self._dmx_target_position = 0
        self._dmx_max_speed = 0
        self._dmx_acceleration = 0
        self._speed = 0
    ...
    def calculate_target_from_dmx(self):
        dmx_coarse = self._dmx_values[0]
        dmx_fine = self._dmx_values[1]        
        target_coarse = self._steps_per_rotation * dmx_coarse /255
        target_fine = self._steps_per_rotation * dmx_fine / 255
        target = target_coarse + target_fine
        return target
        
    def calculate_next_step_speed(self, target, current, max_speed, acceleration):
        if target == current:
            return 0
        if acceleration == 0:
            return max_speed
        ...
        if delta < 0:
            delta = -delta
        speed = math.sqrt(2 * acceleration * delta)
        if speed > max_speed:
            speed = max_speed
        return speed
    
    def calculate_speed_from_dmx(self):
        speed = self._dmx_values[2] / 255 * 100
        min_step_pwm = min(speed, 100)
        return min_step_pwm
    
    def calculate_acceleration_from_dmx(self):
        acceleration = self._dmx_values[3] / 255 * 100
        dmx_acceleration = min(acceleration, 100)
        return dmx_acceleration
    

    def check_endless_rotation(self):
        if self._dmx_values[0] == 255:
            if self._dmx_values[1] == 255:
                rotation = "left"
                return True, rotation
        elif self._dmx_values[0] == 0:
            if self._dmx_values[1] == 0:
                rotation = "right"
                return True, rotation
        return False, None
    ...
    def set_dmx_value(self, channel, value):
        self._dmx_values[channel] = value
