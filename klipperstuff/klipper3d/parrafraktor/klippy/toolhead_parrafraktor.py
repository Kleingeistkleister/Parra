...
import mcu, chelper, kinematics , kinematics.extruder
...
class ToolheadControl():
    ...
    def start_control(self):
        print("Toolhead control started. Use arrow keys to move. Press 'q' to quit.")
        while True:
            command = input("Enter command: ")
            if command.lower() == 'q':
                print("Exiting toolhead control.")
                break
            elif command.lower() in ['up', 'down', 'left', 'right']:
                self.handle_movement(command.lower())
            else:
                print("Invalid command. Please use arrow keys to move or 'q' to quit.")

    def handle_movement(self, direction):
        speed = 100  # Set the speed for movement (adjust as needed)
        ...
        self.toolhead.manual_move(new_position, speed)
